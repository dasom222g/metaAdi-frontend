      ...
      <div className="py-7 px-4 sm:px-6 lg:px-8 fixed top-0 left-0 right-0 z-20 bg-aid-purple">
        ...
        >
          ...
          <div className="hidden md:block md:ml-10 md:pr-4 md:space-x-8">
            {navigation.map((item) => (
              <Link to={item.link} className="font-medium text-white hover:text-aid-blue transition-all duration-300">
                {item.name}
              </Link>
            ))}
            {/* <button
            ...
            >
              <div className="rounded-lg shadow-md overflow-hidden">
                ...
                <div className="px-2 pt-2 pb-3 space-y-1 bg-white bg-opacity-90">
                  {navigation.map((item) => (
                    <Link to={item.link} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700">
                      {item.name}
                    </Link>
                  ))}
                </div>
